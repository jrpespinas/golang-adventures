...
func handler(w http.ResponseWriter, r *http.Request) {
	...
	// retrun solution
	message := computeDeterminants(coefficients)
	fmt.Fprintf(w, "\n%v", message)
}
...
func computeDeterminants(coefficients []int) string {
	...
	if d != 0 {
		...
		return fmt.Sprintf("Dependent - With Multiple Solutions")
	} else {
		return fmt.Sprintf("Inconsistent - No Solution")
	}
}
